...
*/ 
SELECT store_id,
	COUNT(inventory_id) AS inventor_items
FROM inventory
GROUP BY store_id;
...
SELECT
	COUNT(DISTINCT film_id) AS unique_titles, #distinct eliminate duplicates
    store_id
FROM  inventory
GROUP BY store_id;
SELECT
...
SELECT 
	film_id, title, replacement_cost AS least_expensive
FROM film
ORDER BY replacement_cost LIMIT 1;

SELECT 
	film_id, title, replacement_cost AS max_expensive
FROM film
